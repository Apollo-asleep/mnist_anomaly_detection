...
from isolation_trees.isolation_simple_net import IsolationTreeEnsemble, find_TPR_threshold
...
def plot_anomalies(X, y, sample_size=256, n_trees = 100, desired_TPR=None, percentile = None, normal_ymax=None, bins=20):
    ...
    # else:
    #     threshold = np.divide(scores, percentile)
...
if __name__ == '__main__': # dask seems to need this
    ...
    DATA_PATH = './DataSets/MNIST/processes/'
    data_loader_train = torch.utils.data.DataLoader(dataset=DATA_PATH+"training.pd",
                                                batch_size = 64,
                                                shuffle = True)

    X = []
    y = []
    for (image, label) in enumerate(data_loader_train):
        if label == anomalynumber:
            y.append(1)
        else:
            y.append(0)
        x = [image,label]
    ...
    X=np.array(X)
    y=np.array(data_loader_test)
    plot_anomalies(X, y, sample_size=BATCH_SIZE, n_trees=n_trees, desired_TPR=None, bins=15)